    ...
    for f in FILES[dsid]:
        ...
        for tag, f in formulas.items():
            ...
            if isinstance(f, dict):
                m_cut  = (tree['rljet_m_comb[:,0]'] > f['m_lo']) & (tree['rljet_m_comb[:,0]'] < f['m_hi'])
                d2_cut = (tree['rljet_D2'] < f['d2'])
                ntrk_cut = (tree['rljet_ungroomed_ntrk500'] < f['ntrk'])
                score_cut = (m_cut & d2_cut & ntrk_cut)
            else:
                score_cut = f
                score_cut = tagger > score_cut(tree['rljet_pt_comb']/1000.)

            # Apply cuts
            target_pass = target[score_cut]
            weight_pass = weight[score_cut]
