...
if __name__ == "__main__":
    ...
    # Add histograms
    for tag in tags:
        data = hists['data_2015']['rljet_log_m_pt'][tag] + \
               hists['data_2016']['rljet_log_m_pt'][tag] + \
               hists['data_2017']['rljet_log_m_pt'][tag] + \
               hists['data_2018']['rljet_log_m_pt'][tag]
        data = Histo(data)
        hist_df.set_hist(( 'data', tag ), 'Hist', data)
        for samp in samples:
            if samp == 'data': continue
            data = hists[samp+'_mc16a']['rljet_log_m_pt'][tag] + \
                   hists[samp+'_mc16d']['rljet_log_m_pt'][tag] + \
                   hists[samp+'_mc16e']['rljet_log_m_pt'][tag]
